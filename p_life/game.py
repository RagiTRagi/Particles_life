...
def update_particles1(pos, vel, types, world_width, world_height, r_max, dt, friction, noise_strength, matrix):
    ...
    # Kräfte berechnen
    forces = calculate_forces1(sorted_pos, sorted_types, cell_starts, cell_counts, cols, rows, matrix, r_max, world_width, world_height)
...
def update_particles2(pos, vel, types, world_width, world_height, r_max, dt, friction, noise_strength, matrix):
    
    # Grid berechnen
    sorted_pos, sorted_vel, sorted_types, cell_starts, cell_counts, cols, rows = quadrantisieren(pos, vel, types, world_width, world_height, r_max)

    # Kräfte berechnen
    forces = calculate_forces2(sorted_pos, sorted_types, cell_starts, cell_counts, cols, rows, matrix, r_max, world_width, world_height)
...
def calculate_forces1(sorted_pos, sorted_types, 
...
@njit(parallel=True, fastmath=True)
def calculate_forces2(sorted_pos, sorted_types, 
    cell_starts, cell_counts, 
    cols, rows, 
    interaction_matrix, r_max, 
    world_width, world_height
...
class Game:
    ...
    def step(self, dt=0.01): # dt kleiner für Stabilität
        self.pos, self.vel, self.types = update_particles2(
